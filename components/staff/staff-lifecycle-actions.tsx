...
type StaffLifecycleActionsProps = {
  canCheckIn: boolean;
  canCheckOut: boolean;
  reservationCode: string;
  submitReceptionCheckIn: (formData: FormData) => void | Promise<void>;
  submitReceptionCheckOut: (formData: FormData) => void | Promise<void>;
};
...
export function StaffLifecycleActions({
  ...
  submitReceptionCheckOut,
}: StaffLifecycleActionsProps) {
    ...
    <>
      ...
      {dialogMode ? (
        <StaffLifecycleDialog
          ...
          submitAction={dialogMode === "check-in" ? submitReceptionCheckIn : submitReceptionCheckOut}
        />
      ) : null}
    </>
  );
...
function StaffLifecycleDialog({
  ...
  const title = mode === "check-in" ? "Are you sure you want to complete check-in?" : "Are you sure you want to complete check-out?";
  const helperText =
    mode === "check-in"
      ? "Confirm the reservation code below before checking this guest in."
      : "Confirm the reservation code below before checking this guest out.";
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="staff-lifecycle-confirmation-title"
    >
      <div className="flex min-h-dvh items-center justify-center px-4 py-8 sm:px-6">
        <div className="relative w-full max-w-[520px] overflow-hidden rounded-[30px] border border-white/65 bg-white/96 shadow-[0_28px_90px_rgba(15,23,42,0.28)] ring-1 ring-slate-200/60">
          ...
          <button
            ...
            aria-label="Close staff lifecycle dialog"
          >
          ...
          <div className="relative px-7 pb-7 pt-10 sm:px-8 sm:pb-8 sm:pt-11">
            {/* This keeps the reception workflow explicit without adding a new page or changing the surrounding layout. */}
            <div className="mx-auto max-w-md text-center">
              <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-amber-700">
                Staff Confirmation
              </p>
              <h2
                id="staff-lifecycle-confirmation-title"
            ...
            <form action={submitAction} className="mt-8 space-y-4">
              ...
              <label className="block rounded-[22px] border border-stone-200/80 bg-stone-50/85 px-4 py-4">
                ...
                <input
                  ...
                  placeholder="Type reservation code"
                  required
                />
              </label>
